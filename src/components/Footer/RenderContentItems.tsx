...
const renderContentli = (contentLi: string[] | number[] ) => {
  return contentLi.map((item) => (
    <div
      key={uuidv4()}
    >
      <Link className={style.footer__linkItem} href="/" >{item}</Link>
    </div>
  ));
};
  ...
  return (
    <div className={style.footer__navigation}>
      ...
      <div className={style.footer__title}>Жанры</div>
        {renderContentli(cont.genres)}</div>
      </div>

      <div className={style.footer__navigation}>

      <div className={style.footer__title}>Страны</div>
      <div className={style.footer__navigation}>{renderContentli(cont.countries)}</div>

      <div className={style.footer__title}>Годы</div>
      <div className={style.footer__navigation}>{renderContentli(cont.years)}</div>
      </div>
      </div>
    </div>
  );
...
const RenderContentItems: React.FC<RenderProps> = ({ ...item }) => {
  ...
  const [currentLink, setCurrentLink] = useState<EventTarget & HTMLAnchorElement>()
  const [value, setValue] = useState(style.dropdown);
  const update = (e:React.MouseEvent<HTMLAnchorElement, MouseEvent>, title: string) => {
  ...
  return (
    <div >
      <div className={style.footer__title + " " + style.stores}>
        ...
        <Link onClick={(e) => update(e, item.title)} href="/">
          {item.title}{" "}
        </Link>
        {item.content ? arrowSvg : ""}
      </div>
      <div >
        <div className={value}>{item.content ? renderContent(item.content)! : ""}</div>
      </div>
    </div>
  );
};
